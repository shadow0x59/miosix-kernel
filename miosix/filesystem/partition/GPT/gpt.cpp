...
std::expected<std::unique_ptr<GPTReader>, ReaderResult>
GPTReader::readGPT(intrusive_ref_ptr<Device> device, bool skipCheckMBR) 
{
    if (!skipCheckMBR)
    {
        ...
        auto mbrReader=std::move(*mbrResult);
    ...
    std::unique_ptr<GPTReader> reader = std::make_unique<GPTReader>(device);
...
//         {
//             auto result=Unicode::putUtf8(&partName[bufIdx], 
//                 partitionEntry.partitionName[i], bufSize);
//             if (result.first!=Unicode::error::OK) 
//             {

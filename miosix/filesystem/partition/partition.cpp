...
> Partition::enumeratePartitions(intrusive_ref_ptr<Device> physicalDevice) 
{
    ...
    {
        auto partitions=std::vector<
            std::pair<intrusive_ref_ptr<Partition>, PartitionType>>(MBR::NUM_OF_PARTITIONS);
        ...
        for (size_t i=0; i<MBR::NUM_OF_PARTITIONS; i++) 
        {
            ...
            partitions[i]=std::make_pair<intrusive_ref_ptr<Partition>, PartitionType>(

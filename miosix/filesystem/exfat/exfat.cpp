...
ExFatFs::ExFatFs(intrusive_ref_ptr<FileBase> disk, uid_t uid, gid_t gid)
...
int ExFatFs::mkfs()
{
    ...
    MKFS_PARM params;
    params.n_fat=2; // this has no effect on exFAT
    params.align=0; // this will retrieve the block size from memory (512)
    params.n_root=0; // this has no effect on exFAT
    params.au_size=0; // let fatfs decide allocation unit for clusters
    params.fmt=FM_EXFAT|FM_SFD; // with SFD fatfs doesn't create partition table

...
int MountHelper::mountRomFs()
{
    ...
    if(base)
    {
        ...
        if(!bin->mountFailed() && fsm.kmount("/bin",bin)==0) ok=true;
    }
...
static intrusive_ref_ptr<FilesystemBase> tryMount(
    intrusive_ref_ptr<FileBase> disk, PartitionType partitionType)
{
